 function [plane] = PlaneDistribute(lons,lats,highs)
N1 = 10;
goss_num = 2;
goss_plane_num = [5,5];
...
r1 = [ R*sin(pi-theta1)*cos(phy); -R*sin(pi-theta1)*sin(phy);R*cos(pi-theta1)];%ephy,etheta,er
 ...
 r2 = [-sin(lons*pi/180),cos(lons*pi/180),0;cos(lats*pi/180)*cos(lons*pi/180),cos(lats*pi/180)*sin(lons*pi/180),-sin(lats*pi/180);...
  ...
  r_h = [6371*sin(plane(2,i)*pi/180)*cos(plane(1,i)*pi/180);...
                6371*sin(plane(2,i)*pi/180)*sin(plane(1,i)*pi/180);...
                6371*cos(plane(2,i)*pi/180)]; 
            hh = norm(r_h-rs');
end
